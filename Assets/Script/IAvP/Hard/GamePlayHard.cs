...
public class GamePlayHard: MonoBehaviour
{
    ...
    */
    ...
    public void recShowPawn(int x, int y)
    {
        ...
        if (board[x, y] == -1)
        {
            ...
            /*
             * Update HARD.IA.board value with current value
            */
            for (int i = 0; i < 8; i++)
            {
                for (int j = 0; j < 8; j++)
                {
                    hardIA.board[i, j] = board[i, j];
                }
            }
            if (round == 1)
            {
                minMaxAlgorithme(board, round);
            }
        }
    }
    ...
    */
    ...
    private int minMaxAlgorithme(int[,] b, int r) 
    {
        hardIA.GetAllPossibilities(board, true);
        List<int> listOfPossibilitie = hardIA.listOfPossibilities;
        if (listOfPossibilitie.Count > 0) 
        {
            for (int index = 0; index < listOfPossibilitie.Count; index++)
            {
                /* The positions is get from index */
                int y = listOfPossibilitie[index] / 8;
                int x = listOfPossibilitie[index] % 8;
                Debug.Log("enter");
                b[x, y] = round;
                if (r == 0) r = 1;
                else r = 0;
                return minMaxAlgorithme(b, r);
            }
        }
        return -1;
    }
